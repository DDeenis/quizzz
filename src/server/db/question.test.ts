...
describe("Questions DAL", () => {
  it("createEmptyQuestion should create an empty question", async () => {
  ...
  it("updateQuestion should update a question", async () => {
    const teacher = await fixtures.createTeacher();
    const test = await fixtures.createTest(teacher.id);
    const question = await createEmptyQuestion(test.id);
  ...
  it("userCanCreateTestQuestion should return true if the user can create a test question", async () => {
    const teacher = await fixtures.createTeacher();
    const admin = await fixtures.createAdmin();
    const test = await fixtures.createTest(teacher.id);

    await expect(userCanCreateTestQuestion(teacher.id, test.id)).resolves.toBe(
      true
    );
  ...
  it("userCanCreateTestQuestion should return false if the user can't create a test question", async () => {
    const teacher = await fixtures.createTeacher();
    ...
    await expect(userCanCreateTestQuestion(teacher2.id, test.id)).resolves.toBe(
      false
    );
    await expect(userCanCreateTestQuestion(student.id, test.id)).resolves.toBe(
      false
    );
  ...
  it("userCanModifyTestQuestion should return true if the user can create a test question", async () => {
    const teacher = await fixtures.createTeacher();
    const admin = await fixtures.createAdmin();
    const test = await fixtures.createTest(teacher.id);
  ...
  it("userCanModifyTestQuestion should return false if the user can't create a test question", async () => {
    const teacher = await fixtures.createTeacher();
    const teacher2 = await fixtures.createTeacher({
      email: "teacher2@test.com",
    });
