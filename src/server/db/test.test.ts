...
describe("Tests DAL", { retry: 2 }, () => {
  ...
  describe("userCanModifyTest", () => {
    it("should return true if user has permissions to modify a test", async () => {
    ...
    it("should return false if user does not have permissions to modify a test", async () => {
      ...
      await expect(userCanModifyTest(teacher2.id, test.id)).resolves.toBe(
        false
      );
  ...
  describe("userCanCreateTest", () => {
    it("should return true if user has permissions to create a test", async () => {
      const [teacher, admin] = await Promise.all([
        fixtures.createUser({
          email: "teacher1@example.com",
          role: UserRole.Teacher,
    ...
    it("should return false if user does not have permissions to create a test", async () => {
  ...
  describe("createEmptyTest", () => {
    it("should create and empty test", async () => {
      const userId = (await fixtures.createTeacher()).id;
      ...
      expect(test.minimumCorrectAnswers).toBe(
        emptyValues.minimumCorrectAnswers
      );
  ...
  describe("createTest", () => {
    it("should create two tests with the same name but different slugs", async () => {
      const userId = (await fixtures.createTeacher()).id;
      const formValues = fixtures.testFormValues();
  ...
  describe("restoreTest", () => {
    it("should restore soft deleted test", async () => {
      const userId = (await fixtures.createTeacher()).id;
      const test = await fixtures.createTest(userId);
